...
class AssuranceSettings(DnacBase):
    ...
    def validate_input(self):
        ...
        temp_spec = {
            ...
            'assurance_issue_resolution': {
                ...
                'priority': {'type': 'str', 'choices': ['P1', 'P2', 'P3', 'P4'], 'required': False},
                'issue_status': {'type': 'str', 'choices': ['ACTIVE', 'RESOLVED', 'IGNORED'], 'required': False},
            ...
            'assurance_execute_suggested_commands': {
                ...
                'entity_value': {'type': 'str', 'required': True}
            }
        }
    ...
    def input_data_validation(self, config):
        ...
        for issue_resol_ignore in ("assurance_issue_resolution", "assurance_ignore_issue"):
            resol_ignore = config.get(issue_resol_ignore)
            if resol_ignore and len(resol_ignore) > 0:
                for each_issue in resol_ignore:
                    # Validate issue_name for search issue
                    ...
                    # Validate site_hierarchy for search issue
                    ...
                    # else:
                    #     errormsg.append("site_hierarchy: Site name Hierarchy is missing in playbook.")

                    # Validate priority for search issue
                    ...
                    # Validate status for search issue
                    ...
                    # Validate device name for search issue
                    ...
                    # else:
                    #     errormsg.append("device_name: Device name  is missing in playbook.")

                    # Validate start time for search issue
                    ...
                    # else:
                    #     errormsg.append("start_datetime: Start date time is missing in playbook.")
                    ...
                    # else:
                    #     errormsg.append("end_datetime: End date time is missing in playbook.")

                    # Validate mac_address for search issue
                    ...
                    # Validate network_device_ip_address for search issue
                    ...
                    # # Validate Category for search issue
                    # category = each_issue.get("category")
                    # category_list = ("onboarding", "connectivity", "connected",
                    #                  "device", "availability", "utilization",
                    #                  "application", "sensor test", "ap anomaly",
                    #                  "system", "beta analytics", "unknown")
                    # if category and category not in category_list:
                    #     errormsg.append("category: Invalid category '{0}' in playbook. "
                    #                     "Must be one of: {1}.".format(category, ", ".join(category_list)))

                    # # Validate device_type for search issue
                    # device_type = each_issue.get("device_type")
                    # device_type_list = ("access point", "border router", "access",
                    #                     "core", "wireless controller", "sensor",
                    #                     "distribution", "wireless", "wired")
                    # if device_type and device_type not in device_type_list:
                    #     errormsg.append("device_type: Invalid Device Type '{0}' in playbook. "
                    #                     "Must be one of: {1}.".format(device_type, ", ".join(device_type_list)))

                    # # Validate severity for search issue
                    # severity = each_issue.get("severity")
                    ...
                    #                     "Must be one of: {1}.".format(severity, ", ".join(severity_list)))

                    # # Validate entity_type for search issue
                    # entity_type = each_issue.get("entity_type")
                    # entity_type_list = ("network device", "client", "application", "sensor")
                    # if entity_type and entity_type not in entity_type_list:
                    #     errormsg.append("entity_type: Invalid Entity Type '{0}' in playbook. "
                    #                     "Must be one of: {1}.".format(entity_type, ", ".join(entity_type_list)))
    ...
    #         if value is not None and key not in ("control_path", "inclusions"):
    ...
    #     Description:
    #         This function get the path trace config input data and create the path trace then return
    #         output as dict data with task id and flow analysis id.
    #     """
    #     payload_data = {}
    #     for key, value in config_data.items():
    ...
    #                 response.get("response", {}).get("request", {}).get("status") == "COMPLETED":
    #                 self.log("Received the path trace response: {0}".format(self.pprint(response)), "INFO")
    #                 return response.get("response")
    ...
    def get_issue_ids_for_names(self, config_data, verify=None):
        ...
        if start_date and end_date:
            payload_data["startTime"], payload_data["endTime"] = self.validate_start_end_datetime(
                start_date, end_date, [])
        ...
        #             family="issues",
        #             function="issues",
        #             params={"startTime": 1733913000000, "endTime": 1733922000000}
        #         )
        #         self.log("Response from the API: {0}".format(self.pprint(response)),
        #                     "INFO")
        ...
        #                 issue_ids = ([issue["issueId"] for issue in all_issues])
        #                 # if verify:
        #                 #     self.issue_resolved.append([issue["issueId"]
        #                 #                                 for issue in all_issues
        #                 #                                 if issue["status"] == "resolved"])
        #                 #     self.issue_ignored.append([issue["issueId"]
        #                 #                                 for issue in all_issues
        #                 #                                 if issue["status"] == "ignored"])
        #                 #     self.issues_active.append([issue["issueId"]
        #                 #                                 for issue in all_issues
        #                 #                                 if issue["status"] == "active"])

        #             else:
        #                 self.msg = "No data received for the issue: {}".format(str(payload_data))
        #                 self.log(self.msg, "INFO")
        #                 break

        #             page_data = response.get("page")
        #             if page_data:
        #                 self.log("Pagination info {0}".format(self.pprint(page_data)), "INFO")
        #                 if int(page_data["count"]) > 1000:
        #                     self.msg = "Issue found {0} more than 1000 records for given configuration: ".format(page_data["count"])
        #                     self.msg = self.msg + "Cannot proceed, add more fillters to proceeds".format(config_data)
        #                     self.log(self.msg, "ERROR")
        #                     self.set_operation_result("failed", False, self.msg, "ERROR").check_return_status()
        #                     break
        #                 elif len(issue_ids) < int(page_data["limit"]) or\
        #                     int(page_data["offset"]) + int(page_data["limit"]) > int(page_data["count"]):
        #                     break
        #                 elif int(page_data["count"]) > int(page_data["limit"]) and\
        #                     int(page_data["offset"]) + int(page_data["limit"]) < int(page_data["count"]):
        #                     payload_data["offset"] = int(page_data["offset"]) + int(page_data["limit"])
        #         else:
        #             self.msg = "No response received from get issue ids"
        #             self.log(self.msg, "INFO")
        #             break
    ...
    def resolve_issue(self, issue_ids):
        ...
        try:
            ...
            #             "response": {
            #                 "successfulIssueIds": [
            #                     "201fb5a1-95f0-4fd6-b999-a4f77fa5f4a8",
            #                     "2aa228b6-4468-46e7-be9b-3c66d32ecabf"
            #                 ],
            #                 "failureIssueIds": [
            #                     "e25a2b3b-1229-40f3-95c2-749d78498a4d"
            #                 ]
            #             },
            #             "version": "1.0"
            #         }
    ...
    def execute_commands(self, config_data):
        """
    ...
    def create_assurance_issue(self, assurance_details):
        ...
        for issue in create_assurance_issue:
            ...
            if response.get("response"):
                ...
                self.result['changed'] = True
                # self.msg = "User Defined Issue Created Successfully"
                # self.set_operation_result("success", True, self.msg, "INFO")
                    ...
                    for each_trace in get_trace:
                        ...
                        if delete_response:
                            self.log("Path trace already exist hence deleted and re-creating for {0} {1}."
                                     .format(str(each_path), str(each_trace)), "INFO")
            ...
            if resol_ignore and len(resol_ignore) > 0:
                for each_issue in resol_ignore:
                    ...
                    if issue_ids and len(issue_ids) > 0:
                        if issue_resol_ignore == "assurance_issue_resolution":
                            self.log("CHECKING: {0}.".format(resol_ignore), "INFO")
                            response = self.resolve_issue(issue_ids)

                            if response and isinstance(response, dict):
                                self.success_list_resolved.append(each_issue)
                                self.issue_resolved.append(response)
                                #self.issue_processed.append(response)
                ...
                if issue_resol_ignore == "assurance_issue_resolution":
                    ...
                    self.log(self.msg, "INFO")
                    self.set_operation_result(self.status, self.changed, self.msg, "INFO",
                                              self.issue_resolved)#.check_return_status()

                if issue_resol_ignore == "assurance_ignore_issue":
                    if len(self.success_list_ignored) > 0:
                        self.msg = "Issue ignored successfully. '{0}'.".format(
                    ...
                    self.log(self.msg, "INFO")
                    self.set_operation_result(self.status, self.changed, self.msg, "INFO",
                                              self.issue_ignored)#.check_return_status()

                return self
        # return self
        execute_commands = config.get("assurance_execute_suggested_commands")
        if execute_commands is not None and len(execute_commands) > 0:
            success_list = []
            self.msg = ""
            self.changed = False
            self.status = "failed"
            response = {}
            for each_command in execute_commands:
                response = self.execute_commands(each_command)

                if response:
                    success_list.append(each_command)
                    self.cmd_executed.append(response)
                else:
                    self.cmd_not_executed.append(each_command)

            if len(success_list) > 0:
                self.msg = "Command executed successfully for {0}.".format(
            ...
            self.log(self.msg, "INFO")
            self.set_operation_result(self.status, self.changed, self.msg, "INFO",
                                      self.cmd_executed).check_return_status()
        self.log("CHECKRETURN status Fail")
        return self
    ...
    def verify_diff_merged(self, config):
            ...
            for each_path in path_trace:
                if len(self.create_path) > 0:
                    for each_trace in self.create_path:
                        self.log("CHECKING {0} {1}".format(self.pprint(each_trace), self.pprint(each_path)), "INFO")
        ...
        for issue_resol_ignore in ("assurance_issue_resolution", "assurance_ignore_issue"):
            resol_ignore = config.get(issue_resol_ignore)
            #success_list, failed_list, response = [], [], []
            response = []
            self.msg = ""
            ...
            if resol_ignore and len(resol_ignore) > 0:
                if issue_resol_ignore == "assurance_issue_resolution":
                    response = {
                        "processed_issues": self.success_list_resolved,
                        "unprocessed_issues": self.failed_list_resolved,
                        "input_isssue_config": resol_ignore,
                        "processed_logs": self.issue_resolved
                    }

                    if self.success_list_resolved == resol_ignore:
                        ...
                        self.msg = "Unable to verify Issue resolution for '{0}'.".format(
                            str(resol_ignore))
                        self.log(self.msg, "INFO")
                        self.set_operation_result("failed", False, self.msg, "ERROR",
                                                  response).check_return_status()
                ...
                if issue_resol_ignore == "assurance_ignore_issue":
                    response = {
                        "processed_issues": self.success_list_ignored,
                        "unprocessed_issues": self.failed_list_ignored,
                        "input_isssue_config": resol_ignore,
                        "processed_logs": self.issue_resolved
                    }

                    if self.success_list_ignored == resol_ignore:
                        self.msg = "Issue ignored verified successfully for '{0}'.".format(
                            str(resol_ignore))
                        self.log(self.msg, "INFO")
                        self.set_operation_result("success", True, self.msg, "INFO",
                                                  response).check_return_status()
                    else:
                        self.msg = "Unable to verify ignored issue for '{0}'.".format(
                            str(resol_ignore))
                        self.log(self.msg, "INFO")
                        self.set_operation_result("failed", False, self.msg, "ERROR",
                                                  response).check_return_status()
                ...
                # try:
                #     for each_issue in resol_ignore:
                #         issue_details = self.get_issue_ids_for_names(each_issue, True)
                #         each_response = {}
                #         if issue_resol_ignore == "assurance_issue_resolution":
                #             if len(self.issue_resolved) > len(self.issues_active):
                #                 success_list.append(self.issue_resolved)
                #             else:
                #                 failed_list.append(self.issues_active)

                #         if issue_resol_ignore == "assurance_ignore_issue":
                #             if len(self.issue_ignored) > len(self.issues_active):
                #                 success_list.append(self.issue_ignored)
                #             else:
                #                 failed_list.append(self.issues_active)

                #         each_response = {
                #             "processed_issue_ids": success_list,
                #             "unprocessed_issue_ids": failed_list,
                #             "isssue_config": each_issue,
                #             "process_logs": self.issue_resolved
                #             }
                #     response.append(each_response)

                #     if len(response) > 0:
                #         if issue_resol_ignore == "assurance_issue_resolution":
                #             self.msg = "Issue resolution verified successfully for '{0}'.".format(str(resol_ignore))
                #         else:
                #             self.msg = "Issue ignored verified successfully for '{0}'.".format(str(resol_ignore))
                #         self.log(self.msg, "INFO")
                #         self.set_operation_result("success", True, self.msg, "INFO",
                #                         response).check_return_status()
                #     else:
                #         self.msg = "Unable to verify reponse for '{0}'.".format(str(resol_ignore))
                #         self.log(self.msg, "INFO")
                #         self.set_operation_result("failed", False, self.msg, "INFO").check_return_status()

                # except Exception as e:
                #     self.msg = 'An error occurred during verification issue: {0}'.format(str(e))
                #     self.log(self.msg, "ERROR")
                #     self.set_operation_result("failed", False, self.msg, "ERROR").check_return_status()
        ...
        if execute_commands is not None:
            ...
            if len(self.cmd_executed) > 0:
                self.msg = "Command executed and verified successfully for {0}.".format(
                    execute_commands)
                self.changed = True
                self.status = "success"
